...
function findFirstByTag(
  parent: ParseParentNode,
  tag: string,
): ParseElement | null {
  const needle = tag;
...
export function parseNavHtml(html: string): MetaPayload {
  const weekSelectMatch = html.match(
    /<select\s+name="week"[\s\S]*?<\/select>/i,
  );
  if (!weekSelectMatch) throw new Error("Week select not found in navbar");

  const weekOptions: WeekOption[] = [];
  for (const match of weekSelectMatch[0].matchAll(
  ...
  const groups: GroupOption[] = parseJsStringArray(classesMatch[1]!).map(
    (codeRaw, idx) => ({
      id: idx + 1,
...
function parseLegendEntries(
  ...
  const cells = Array.from(
    tableMatch[1]!.matchAll(/<TD[^>]*>([\s\S]*?)<\/TD>/gi),
...
function parseDayDates(html: string, weekStartDateIso: string): string[] {
  const match = [
    ...html.matchAll(
      /<B>\s*[А-Яа-яA-Za-z]+\s+(\d{1,2})\.(\d{1,2})\.?\s*<\/B>/gi,
    ),
  ];
...
function extractCellLines(td: ParseElement): string[] {
  const nestedTable =
    directChildrenByTag(td, "table")[0] ?? findFirstByTag(td, "table");
  if (!nestedTable) {
...
export function parseTimetablePage(
  ...
  const rowTimes: Array<{ start: string; end: string } | null> = new Array(
    rows.length,
  ).fill(null);
  const dayDates = parseDayDates(html, week.startDateIso);

  const subjectResolver = makeLegendResolver(
    parseLegendEntries(html, "Дисциплины"),
  );
    ...
    for (const cell of cells) {
      ...
      if (col === 0) {
        ...
        if (periodRange) {
          for (
            let r = rowIndex;
            r < Math.min(rows.length, rowIndex + rowSpan);
            r += 1
          ) {
        ...
        if (subjectRaw && !subjectRaw.includes("________________")) {
          ...
          if (dayIndex >= 0 && dayIndex < dayDates.length) {
            ...
            const endRange =
              rowTimes[Math.min(rows.length - 1, rowIndex + rowSpan - 1)] ??
              startRange;
            if (startRange && endRange) {
              ...
              const track =
                nameTrack !== "none" ? nameTrack : (cohort?.track ?? "none");
              const cohortCode =
                cohort && !genericCodes.has(cohort.code) ? cohort.code : null;
